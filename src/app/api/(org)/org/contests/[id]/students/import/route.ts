...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  return withApiLogging("org.contests.students.import", async () => {
    const limited = checkRequestRateLimit(request, "orgWrite", ["contest-students-import", params.id]);
    ...
    const res = await callGoApi("POST", `/org/contests/${params.id}/students/import`, payload as Record<string, unknown>, auth.uid);
    if (res.status !== 200) {
      return apiError(
        safeGoApiError(res, "Unable to import students."),
        res.status,
