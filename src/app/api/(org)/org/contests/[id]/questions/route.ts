...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  return withApiLogging("org.contest_questions.create", async () => {
    ...
    const res = await callGoApi("POST", `/org/contests/${params.id}/questions`, payload as Record<string, unknown>, auth.uid);
    if (res.status !== 201) {
      return apiError(
        typeof res.data === "object" && res.data && "error" in res.data ? String((res.data as { error: unknown }).error) : "Unable to create question.",
        res.status,
    ...
    return apiOk({ saved: true, id: data.id }, { status: 201 });
  });
}
