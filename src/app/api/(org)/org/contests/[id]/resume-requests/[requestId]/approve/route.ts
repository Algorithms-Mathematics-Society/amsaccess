...
export async function POST(
  ...
  return withApiLogging("org.contests.resumeRequests.approve", async () => {
    ...
    const res = await callGoApi(
      "POST",
      `/org/contests/${params.id}/resume-requests/${params.requestId}/approve`,
      body,
      auth.uid
    );
    if (res.status !== 200) {
      return apiError("Unable to approve resume request.", res.status, "SERVER_ERROR");
    }
