...
export async function POST(request: NextRequest) {
  const limited = await checkRequestRateLimitAsync(request, "orgWrite", ["judge-capacity-stop"]);
    ...
    if (res.status !== 200) {
      const code =
        typeof res.data === "object" && res.data && "code" in res.data
          ...
          : "SERVER_ERROR";
      return apiError(safeGoApiError(res, "Unable to stop judge capacity."), res.status, code);
    }
